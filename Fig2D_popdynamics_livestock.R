...
spp.names = c("Haemonchus contortus", "Ostertagia circumcincta", "Trichostrongylus colubriformis")
...
for(i in 1:length(parms.listL)){
  parms <- parms.listL[[i]]
  species_stub <- spp.names[i]
  
  
  n0<-c(W=100,L=3000, x=0, y=0) #set initial conditions
  tf<-100
  times<-0:tf
  treat.times <- round(seq(10, tf, by=10))
...
p1 <- ggplot(data=ans.all, aes(x=time, y=W, col=species)) + 
  ...
  ylab("Adult worms") + xlab("Time in days") + ylim(0, 700) +xlim(0, 70) +
  scale_color_manual(values=c("#B79F00", "#00BFC4", "#F564E3"), labels = c(
...
p1
save_plot("Fig2D.pdf", p1, base_height=3, base_width=6)
